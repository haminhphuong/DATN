...
namespace Ecommage\AutoGenerateMetaContent\Helper;
...
class Html2Text
{
     ...
     * @return string
     */
    public function getShortText($limit = 255, $addDots = false)
    {
        $text = $this->getText();
        if (strlen($text) <= $limit) {
            return $this->addEndCharacter($text, $addDots);
        }

        $orgText   = preg_replace('/\s+/', ' ', $text);
        $shortText = substr($orgText, 0, $limit);
        $endChart  = substr($shortText, -1);
        if (in_array($endChart, [' ', '.', ','])) {
            return $this->addEndCharacter($shortText, $addDots);
        }
     ...
     * @return mixed|string
     */
    public function addEndCharacter($text, $addDots)
    {
        if ($addDots !== false) {
            $text .= $addDots;
        }
