    ...
    $.widget('mage.amLocator', {
        ...
        display: function() {
            var country = $('select[name="country"]').val(),
                ...
                self = this,
                mapOptions = {zoom: 4};
            var regionName = $('input[name="state"]').val();
            var region = $('select[name="state_id"]').val();
            var city_id = $('select[name="city_id"]').val();

            if (region) {
                var regions = regionJson[country];
                regionName = regions[region].name;
            }

            if (city_id) {
                var citys = cityJson[region];
                city = citys[city_id].name;
            }

            address = country +','+ regionName + ',' + city+','+zip+','+address;
            document.getElementById("map-canvas").style.display = "block";
