...
namespace Ecommage\CustomStoreLocation\Model\Import;
...
class Location extends AbstractEntity
{
    ...
    public function __construct(
        ...
        \Amasty\Storelocator\Model\Import\Validator $validator,
    ...
    protected function saveLocation(array $locations, $table)
    {
        ...
        foreach ($locations as $location) {
            ...
            $locationModel->save();
            if (isset($location['is_base']) && is_array($location['is_base'])) {
                $this->saveNewGallery($location);
            }
        }
     ...
     * @throws \Exception
     */
    public function saveNewGallery($data)
    {
        $locationId = $data['id'];
        $allImages  = $this->_galleryCollection->getImagesByLocation($locationId);
        ...
        foreach ($galleryImages as $key => $galleryImage) {
            ...
            if (isset($galleryImage['tmp_name']) && isset($galleryImage['name'])) {
                ...
                $newImageCollection = $newImage->getCollection()->addFieldToFilter('location_id', $locationId);
                $isBase = $newImage->getCollection()->addFieldToFilter('location_id', $locationId)->addFieldToFilter('is_base', 1);
                if ($isBase->getSize() === 0) {
                    $newImage->load($newImageCollection->getFirstItem()->getData('id'))->setData('is_base', $data['is_base'][$key])->save();
                }
            }
        }
    }
     ...
     * @return int|string
     */
    private function checkCity($city, $region, $country)
    {
        /** @var \Amasty\Storelocator\Model\Import\Validator\Country $validator */
        $validator = $this->_getValidator(self::VALIDATOR_COUNTRY);
        ...
        if (strlen($region) > 0) {
            $region = $validator->getRegionByName($region, $country);
            if ($region && strlen($city) > 0) {
                $city = $validator->getCityByName($city, $region);
            }
        }
    ...
    private function prepareCustomRows($key, $locationData, $rowNum)
    {
        ...
        switch ($key) {
            ...
            case self::COL_CITY:
                if (isset($locationData[self::COL_COUNTRY]) && isset($locationData[self::COL_STATE])) {
                    $cityId =  $this->checkCity($locationData[$key], $locationData[self::COL_STATE], $locationData[self::COL_COUNTRY]);
                    if ($cityId !== $locationData[$key]) {
                        $locationData[self::COL_CITY_ID] = $cityId;
                    } else {
                        $locationData[self::COL_CITY] = $cityId;
                    }
                }
                break;
            ...
            case self::COL_IS_BASE:
                $isBases            = explode(',', $locationData[$key]);
                $locationData[$key] = [];
                foreach ($isBases as $isBase) {
                    $locationData[$key][] = $isBase;
                }
                break;
