...
namespace Ecommage\CustomStoreLocation\Plugin\Helper;
 ...
 */
class RegionData extends \Magento\Directory\Helper\Data
{
    /**
        ...
        if (!$this->_regionJson) {
            ...
            if (empty($json)) {
                if ($this->_request->getFullActionName() == 'catalog_product_view') {
                    $regions = $this->getRegionDataProductPage();
                } else {
     ...
     * @return array
     */
    public function getRegionDataProductPage()
    {
        $scope = $this->getCurrentScope();
        ...
        foreach ($collection as $region) {
            ...
            if (!$region->getRegionId() || !in_array($region->getRegionId(), $this->getExistRegions($collection)) ) {
                continue;
            }
            $result = str_replace('Thành phố', 'Tp', $region->getName());
            $regions[$region->getCountryId()][$region->getRegionId()] = [
                'code' => $region->getCode(),
                'name' => (string)__($result),
            ];
        }
     ...
     * @return array
     */
    public function getExistRegions($collection)
    {
        $connection   = $collection->getConnection();
        $tableCheck = $collection->getTable('amasty_amlocator_location');
        $sql = $connection->select()->distinct(true)->from($tableCheck, ['state']);
