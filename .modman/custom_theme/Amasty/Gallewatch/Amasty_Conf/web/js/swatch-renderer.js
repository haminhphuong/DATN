    ...
    function ($, _, utils) {
        ...
        $.widget('amasty_conf.SwatchRenderer', $.mage.SwatchRenderer, {
            ...
            _EventListener: function () {
                ...
                var $widget = this;
                $widget.element.on('click', '.swatch-option-container', function () {
                    $(this).prevAll().prop('disabled', false);
                    $(this).nextAll().prop('disabled', false);
                    $(this).prop('disabled', true);
                    return $widget._AmOnClick($(this).find('.'+ $widget.options.classes.optionClass), $widget);
                });
            ...
            _addOutOfStockLabels: function () {
                ...
                $.each(attributeJson.options, function () {
                    ...
                    option = $widget.element.find('[data-option-label="'+this.label+'"]');
                    if (product && option.length) {
                        if (productInformation[product] && !productInformation[product].is_in_stock) {
                            option.removeClass('selected')
                                .parents('.swatch-option-container')
                                .addClass('out-of-stock')
                                .addClass('disabled')
                                .attr('disabled', 'disabled');
                        } else {
                            option.parents('.swatch-option-container')
            ...
            _RenderPricesForControls: function () {
                ...
                $('[attribute-id="' + attributeJson.id + '"] .swatch-option-label, '
                    + '[data-attribute-id="' + attributeJson.id + '"] .swatch-option-label').remove();
                    ...
                    if (product) {
                        ...
                        if (price) {
                            ...
                            if (option.length && formatted) {
                                ...
                                option.after('<span class="swatch-option-label">' + this.label + '</span>' );
            ...
            _RenderControls: function () {
                ...
                var countAttr = this.options.jsonConfig.attributes.length;
                var checkout = 0;
                $widget.optionsMap = {};
                ...
                $.each(this.options.jsonConfig.attributes, function () {
                    ...
                    $.each(item.options, function (key, data) {
                        ...
                        var option = $('.swatch-option[option-id="' + this.id + '"], .swatch-option[data-option-id="' + this.id + '"]');

                        if(checkout < countAttr){
                            if (option.length) {
                                if (option.parents('.swatch-option-container').length === 0) {
                                    option.wrap("<div class='swatch-option-container'></div>");
                                }
                ...
                controls.each(function () {
                    ...
                    $this.find('[option-id], [data-option-id]').each(function () {
                        ...
                        if (_.intersection(products, $widget.optionsMap[id][option].products).length <= 0) {
                            ...
                            $element.parent().addClass('master_disable');
                            ...
                            $element.parent().removeClass('master_disable');
                        }
                    });
                });
            },
